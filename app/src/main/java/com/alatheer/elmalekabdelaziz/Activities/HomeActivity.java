package com.alatheer.elmalekabdelaziz.Activities;
...
public class HomeActivity extends AppCompatActivity implements OnMenuItemClickListener{
    ...
    private Fragment_Register_kafeel  fragment_register_kafeel;
        ...
        switch (register_type)
        {
            ...
            case Tags.kafeel:
                ...
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        fragment_register_kafeel = Fragment_Register_kafeel.getInstance();
                        getSupportFragmentManager().beginTransaction().replace(R.id.fragments_container,fragment_register_kafeel).commit();
                        tv_title.setText("Register");
                        behavior.setState(BottomSheetBehavior.STATE_EXPANDED);
                    }
                },700);
